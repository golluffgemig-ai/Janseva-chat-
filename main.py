...
def init_db():
    ...
    columns = [("users", "bio", "TEXT DEFAULT 'Hey there!'"),
               ("users", "lock_enabled", "INTEGER DEFAULT 0"),
               ("users", "screen_pin", "TEXT DEFAULT ''")]
...
def get_profile():
    ...
    row = c.fetchone()
    conn.close()
    return jsonify({'bio': row[0], 'lock_enabled': row[1], 'screen_pin': row[2]}) if row else jsonify({})
